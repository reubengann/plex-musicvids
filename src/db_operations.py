...
from src.models import (
    ...
    PlexMediaItem,
    PlexMediaPart,
    PlexPlayQueueGenerator,
)

PlexVideo = namedtuple("PlexVideo", ["media_item", "media_part"])
PlexPlaylistVideo = namedtuple(
    "PlexPlaylistVideo", ["media_item", "media_part", "queue_generator"]
)
...
def get_playlists(session):
    return (
        session.query(PlexMetaDataItem)
        .filter(PlexMetaDataItem.metadata_type == PLAYLIST_METADATA_TYPE)
        .all()
    )
...
def get_videos_in_plex(session, vids_folder):
    results = (
        session.query(PlexMediaItem, PlexMediaPart)
...
def get_videos_in_playlist(session, playlist_id):
    results = (
        session.query(PlexMediaItem, PlexMediaPart, PlexPlayQueueGenerator)
        .join(PlexMediaPart, PlexMediaItem.id == PlexMediaPart.media_item_id)
        .join(
            PlexPlayQueueGenerator,
            PlexPlayQueueGenerator.metadata_item_id == PlexMediaItem.metadata_item_id,
        )
